...
export class AbsencesPage implements OnInit{
  ...
  confirm() {
    if (this.reason && this.datevon && this.datebis) {
      ...
      this.http.get('http://192.168.244.92:3000/api/absences/add?datevon=' + this.datevon + '&datebis=' + this.datebis + '&reason=' + encodedReason +'&uid=' +this.cookieService.get('uid')).subscribe(
        (response) => {
  ...
  getAbsences() {
    this.http.get<any[]>('http://192.168.244.92:3000/api/user/absences?uid= '+   this.cookieService.get('uid')).subscribe(
      (response) => {
